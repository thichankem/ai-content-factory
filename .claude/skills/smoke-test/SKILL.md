...
runs on :8080, waits for `/health`, then asserts the complete pipeline:

1. `GET /health` returns `ok`
2. `POST /projects` creates a draft project
3. `PUT /projects/{id}/script` + rights → `script_review`
4. `POST /projects/{id}/approvals` (script) → `script_approved`
5. `POST /projects/{id}/generate` → `generating`
6. Waits for the worker → `video_review` with video metadata + progress 100
7. `POST /projects/{id}/approvals` (video) → `video_approved`
8. `POST /projects/{id}/publish` → `published`
9. Final GET confirms the state machine is consistent
10. `GET /` serves the frontend HTML and the thumbnail SVG is reachable
