...
### Example: Tool Execution via curl

```bash
# 1. Create a project
curl -s -X POST http://127.0.0.1:8080/tools/call \
  -H 'Content-Type: application/json' \
  -d '{"tool": "create_project", "args": {"name": "Mars Mission", "topic": "Sao Hoa", "target_language": "vi", "duration_target_seconds": 60}}'

# 2. Get video contact sheet for visual inspection
curl -s -X POST http://127.0.0.1:8080/tools/call \
  -H 'Content-Type: application/json' \
  -d '{"tool": "media_contact_sheet", "args": {"ref": "storage/uploads/video.mp4", "num_frames": 6, "columns": 3}}'
```
...
### Registered MCP Tools (21+ Core Tools)
- `media_list`, `media_get`, `media_upload`, `media_transcribe`, `media_recook`, `media_extract_text`.
- `video_cut_clip`, `video_concat_clips`, `video_detect_scene_cuts`, `video_score_best_frame`.
- `voice_synthesize_speech`, `image_crop`, `image_remove_background`, `image_upscale`.
- `audio_detect_silence_and_pace`, `audio_check_quality`, `audio_detect_speech_emotion`, `audio_classify_music_mood`, `audio_describe_scene`, `audio_detect_events`, `audio_diarize_speakers`.
...
3. **Agent Responds**: Agent outputs structured Markdown blocks:
   ````markdown
   ```script
   [Hook]
   Dưới đáy biển sâu 3.800 mét...
   ```
   ```json scenes
   [{"label": "Hook", "text": "Titanic", "narration": "Dưới đáy biển sâu..."}]
   ```
   ````
